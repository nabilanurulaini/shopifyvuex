<template>
  <div class="cart">
    <div>
      ...
      <v-alert v-if="!cart.length" color="secondary">No Product in cart!</v-alert>
      <v-list>
        <!-- Daftar item dalam keranjang -->
        <v-list-item v-for="item in cart" :key="item.id">
          <!-- Konten item dalam keranjang -->
          <v-list-item-content>
          <v-img :src="item.thumbnail" class="white--text align-end"></v-img>
...
export default {
  ...
  methods: {
    ...mapActions(["removeItemFromCart", "addQty", "reduceQty", "emptyCart", "placeOrder"]),
    async placeOrder() {
